...
@section('content')
	...
	<div class="row">
		...
		<form role="form" method="POST" action="{{ route('pcv.update', $pcv->id) }}" class="col-lg-12" id="pcv_form">
			@csrf
			@method('PUT')
			...
			<input type="hidden" name="pcv_accounts" id="pcv_accounts" 
				value="{{ old('pcv_accounts', $pcv->account_transactions) }}" />
			...
			<div class="col-lg-12 mg-b-20">
				<div class="custom-control custom-checkbox">
					<input type="checkbox" class="custom-control-input static-inputs" id="with-temporary-slip" name="withslip"
						@if(old('withslip') || !is_null($pcv->slip_no)) checked @endif>
				...
				<div class="form-group row">
					...
					<div class="col-lg-7">
						<input list="tsNo" type="text" class="form-control static-inputs" id="ts_no" name="ts_no"
							@if(!old('withslip')) disabled @endif value="{{ old('ts_no', $pcv->ts_no) }}">
						<datalist id="tsNo">
				...
				<div class="form-group row">
					...
					<div class="col-lg-7">
						<input type="number" class="form-control static-inputs" id="change" name="change" 
							value="{{ old('change', $pcv->change) }}" step="1" min="0" placeholder="0.00">
					</div>
				</div>
			</div>
	            	...
	            	<div class="col-lg-7">
						<select class="custom-select static-inputs" id="account_name" name="account_name">
							...
							@foreach( \App\Models\Account::getAccounts() as $account )
								<option value="{{ $account['name'] }}" @if(old('account_name', $pcv->account_name)==$account['name']) selected @endif> 
									{{ $account['name'] }} 
								</option>
							@endforeach
						</select>
			...
			$.each(_account_transactions, function(i, data){
				...
				if($('#btn-add-account-details').length > 0) {
					...
					$('.custom-inputs').each(function(i, d) {
						$(this).val(_data[$(this).attr('data-name')]);
					});
