	          	...
	          	<div class="form-group row">
	            	...
	            	<div class="col-lg-7">
						<select class="custom-select static-inputs" id="account_name" name="account_name">
							...
							@php $isOthers = true; @endphp @endphp
							@foreach( \App\Models\Account::getAccounts() as $account )
								@if(!$account['others'])
								@php if(old('account_name', $pcv->account_name)==$account['name']){ $isOthers = false; } @endphp
								...
								@endif
							@endforeach
							<option value="others" @if($isOthers) selected @endif> Others </option>
						</select>
						<select class="mg-t-20 custom-select static-inputs @if($errors->has('account_name_other')) is-invalid @endif @if($isOthers) show @else hide @endif" 
							id="account_name_other" name="account_name_other">
							<option value="">Select Account</option>
							@foreach( \App\Models\Account::getAccounts() as $account )
								@if($account['others'])
								<option value="{{ $account['name'] }}" @if(old('account_name_other', $pcv->account_name)==$account['name']) selected @endif> 
									{{ $account['name'] }} 
								</option>
								@endif
							@endforeach
...
@section('pagejs')
	...
	<script type="text/javascript">
		...
		$(document).on('focusout', '#bill_date_to', function(){
		...
		function getAccountTransactions() {
			...
			if($('#btn-add-account-details').length > 0) {
					...
					if( _hasError == "1") {
						...
						if($(this).attr('data-name') == 'brand') {
							...
							$.ajax({
								...
								url 	: '{!! env("APP_URL") !!}' + '/job-request/search?search='+_account_transactions[0][0]['job_request_no'] ,
								method 	: 'GET' ,
								success : function (res) {
									$('.custom-inputs[data-name="brand"]').empty();
									if( res.length > 0 ) {
										...
										$.each(res, function(i, o) {
											if( o.brand ==  _account_transactions[0][0]['brand'] ) {
												_options += '<option value="'+o.brand+'" selected>'+o.brand+'</option>';
											} else {
												_options += '<option value="'+o.brand+'">'+o.brand+'</option>';
											}
				...
				if(_account_name == 'Installation') {
					...
					if( _hasError != "1") { 
						$.each(_account_transactions[0][0]['items'], function(qq, ff) {
							...
							$('.tbl-header').each(function(ii, res) {
								...
								if( _row_name != 'action') { 
									_html += '<td data-name="'+_row_name+'" >' + _account_transactions[0][0]['items'][qq][$(this).data('rowname')] + '</td>';	
								}
